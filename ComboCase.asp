      ...
      <div id="main">
	   ...
	   <!--#include file="banner.asp"-->
        ...
        <div id="pros" class="prolist">
		 ...
		 <%
		sqllar="select * from PClass where gb='"&gbbase(0)&"' and ParentID=370 order by Orders asc"
		Set rs=Server.CreateObject("ADODB.RecordSet") 
		rs.Open sqllar,conn,1,1 
		if not rs.eof then
		do while not rs.eof 
		%>
            <li><a href="ComboCase.asp?parentid=<%=rs("id")%>" title="<%=rs("classname")%>"><%if ptitle = rs("classname") then%><font color="#FF3300"><%=rs("ClassName")%></font><%else%><%=rs("ClassName")%><%end if%></a></li>
		<%
		rs.movenext
		loop
		end if
		...
		set rs = nothing
		%>
            <li class="view_all"><a href="ComboCase.asp" title="View all Phone Cases">View All</a></li>
          </ul>

          <!--#include file="sharenew/prodlist_for_combo.asp" -->
