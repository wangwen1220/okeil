    ...
    <div id="content">
      <div id="main">
	   ...
	   <!--#include file="banner.asp"-->
		...
		do while not rs.eof 
		%>
            <li><a href="Newleather.asp?parentid=<%=rs("id")%>" title="<%=rs("classname")%>"><%if ptitle = rs("classname") then%><font color="#FF3300"><%=rs("ClassName")%></font><%else%><%=rs("ClassName")%><%end if%></a></li>
		<%
		rs.movenext
		loop
		end if
		...
		set rs = nothing
		%>
            <li class="view_all"><a href="Newleather.asp" title="View all Phone Cases">View All</a></li>
          </ul>

          <!--#include file="sharenew/prodlist_for_leather.asp" -->
