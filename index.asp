    ...
    <div id="content">
      <div id="main">
	   ...
	   <!--#include file="banner.asp"-->
        <h2 id="position">Welcome to <a href="/index.asp" title="LIEKO Mobile Phone Case">LIEKO</a></h2>

        <div id="pros" class="prolist">
          <h1>Mobile Phone Case List</h1>
          <p class="notice">Check the individual product information , Please visit the <strong>"Hot Selling"</strong> section.</p>
          <p class="notice">You can search products with the product number in the search box, for an example, search item#: IP-4-A</p>
		...
		do while not rs.eof 
		%>
            <li><a href="index.asp?parentid=<%=rs("id")%>" title="<%=rs("classname")%>"><%if ptitle = rs("classname") then%><font color="#FF3300"><%=rs("ClassName")%><%else%><%=rs("ClassName")%><%end if%></a></li>
		<%
		rs.movenext
		loop
		end if
		...
		set rs = nothing
		%>
            <li class="view_all"><a href="index.asp" title="View all Phone Cases">View All</a></li>
          </ul>

          <!--#include file="sharenew/prodlist_for_index.asp" -->
