...
<%do while irecordsshown<MaxPage and NOT rsprod.EOF%>
       ...
       <div class='new box'>
            <a href="productdetail.asp?ProdNum=<%=rsprod("ProdNum")%>&parentid=<%=rsprod("gbseq")%>&larseq=<%=rsprod("larseq")%>"><img src='<%=rsprod("photo1")%>' alt="<%=rsprod("ProdID")%>|<%=rsprod("larcode")%>|<%=rsprod("searchtype")%>" border=0 width="150" height="150"></a>
            <h3><a href="productdetail.asp?ProdNum=<%=rsprod("ProdNum")%>&parentid=<%=rsprod("gbseq")%>&larseq=<%=rsprod("larseq")%>"><%=left(rsprod("prodid"),30)%></a><span>Product Number:<%=rsprod("model")%></span></h3>
          </div>
