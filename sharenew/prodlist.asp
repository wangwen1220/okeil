...
num=rsprod("prodnum")
response.Redirect ("productdetail.asp?ProdNum="&num&"&parentid="&parentid&"&larseq="&larseq&"")
end if 
...
<%do while irecordsshown<MaxPage and NOT rsprod.EOF%>
    <li>
        <h4><a href="productdetail.asp?ProdNum=<%=rsprod("ProdNum")%>&parentid=<%=rsprod("gbseq")%>&larseq=<%=rsprod("larseq")%>"><img src='<%=rsprod("photo1")%>' alt="<%=rsprod("ProdID")%>|<%=rsprod("larcode")%>|<%=rsprod("searchtype")%>" border=0></a></h4>
		<p align="center"><a href="productdetail.asp?ProdNum=<%=rsprod("ProdNum")%>&parentid=<%=rsprod("gbseq")%>&larseq=<%=rsprod("larseq")%>"><%=left(rsprod("prodid"),30)%></a></p>
		<p align="center">Product Number:<%=rsprod("model")%></p>
    </li>
<%
