...
<%do while irecordsshown<MaxPage and NOT rsprod.EOF%>
        ...
        <div class="box"> <a href="productdetail_img_sp.asp?ProdNum=<%=rsprod("ProdNum")%>&parentid=<%=rsprod("gbseq")%>&larseq=<%=rsprod("larseq")%>"><img src='<%=rsprod("photo1")%>' alt="<%=rsprod("ProdID_sp")%>|<%=rsprod("larcode_sp")%>|<%=rsprod("searchtype")%>" border=0></a> 
