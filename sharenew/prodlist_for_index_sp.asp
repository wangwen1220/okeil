...
sqlprod="select * from ProdMain where gb='"&gbbase(0)&"' and larseq='243' "
...
<%do while irecordsshown<MaxPage and NOT rsprod.EOF%>
        ...
        <div class="box"> <a href="productdetail_img_sp.asp?ProdNum=<%=rsprod("ProdNum")%>&parentid=<%=rsprod("gbseq")%>&larseq=<%=rsprod("larseq")%>"><img src='<%=rsprod("photo1")%>' alt="<%=rsprod("ProdID")%>|<%=rsprod("larcode")%>|<%=rsprod("searchtype")%>" border=0></a> 
          <h3><a href="productdetail_img_sp.asp?ProdNum=<%=rsprod("ProdNum")%>&parentid=<%=rsprod("gbseq")%>&larseq=<%=rsprod("larseq")%>"><%=left(rsprod("prodid_sp"),30)%></a></h3>
        </div>
